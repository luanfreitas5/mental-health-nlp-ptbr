...
def compute_sentiment_trend(tweets: pl.DataFrame, config: TemporalSection) -> pl.DataFrame:
    ...
    for (user_id,), user_frame in (
        ...
        # Janela recente: o estado atual do usuário pesa mais que a média de
        # um ano inteiro para uma decisão de triagem.
        horizon = float(days.max()) - config.recent_window_days
        ...
        if recent_mask.sum() >= 3:
            ...
            record[f"{TEMPORAL_PREFIX}polarity_recent_delta"] = recent_mean - float(polarity.mean())
        else:
            record[f"{TEMPORAL_PREFIX}polarity_recent_mean"] = float("nan")
            record[f"{TEMPORAL_PREFIX}polarity_recent_delta"] = float("nan")
        ...
        if config.negative_persistence:
            ...
            record[f"{TEMPORAL_PREFIX}negative_streak_days"] = float(best)
        ...
        if config.polarity_shift:
            ...
            record[f"{TEMPORAL_PREFIX}monthly_polarity_std"] = (
                float(np.nanstd(values)) if len(values) > 1 else float("nan")
            )
            record[f"{TEMPORAL_PREFIX}monthly_polarity_shift"] = (
                float(values[-1] - values[0]) if len(values) > 1 else float("nan")
            )
...
def compute_risk_intensification(tweets: pl.DataFrame, config: TemporalSection) -> pl.DataFrame:
    ...
    frame = tweets.with_columns(
        pl.col(TEXT_CLEAN).map_elements(has_risk, return_dtype=pl.Boolean).alias("_risk")
    ).sort(CREATED_AT)
